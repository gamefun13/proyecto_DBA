...
\begin{center}
\path{rooftop_by_municipio_deduplicated.csv}
\end{center}
...
\begin{table}[H]
\centering
\caption{Comparación numérica entre Google Open Buildings y Microsoft Building Footprints}
\begin{tabular}{p{6cm} r r}
\toprule
\tblhdr{Métrica} & \tblhdr{Google} & \tblhdr{Microsoft} \\
\midrule
Edificios cargados & 2,686,933 & 1,537,288 \\
\rowcolor{grisfilas}
Edificios duplicados entre fuentes & -- & 764,327 \\
Edificios únicos aportados al resultado final & 2,686,933 & 772,961 \\
\rowcolor{grisfilas}
Total consolidado sin duplicados & \multicolumn{2}{r}{3,459,894} \\
\bottomrule
\end{tabular}
\end{table}
...
\tblhdr{Métrica} & \tblhdr{Google} & \tblhdr{Microsoft} \\
\midrule
Participación sobre el total procesado & 63.6\% & 36.4\% \\
\rowcolor{grisfilas}
Participación sobre el resultado final sin duplicados & 77.7\% & 22.3\% \\
\bottomrule
\end{tabular}
\end{table}
...
A partir del análisis realizado y de la arquitectura geoespacial propuesta, se recomienda a la Unidad de Planeación Minero Energética (UPME):

\begin{itemize}
    \item Utilizar Google Open Buildings como fuente base para la identificación inicial de edificaciones dentro de municipios PDET debido a su amplia cobertura territorial.
    \item Complementar el análisis mediante muestras de validación utilizando Microsoft Building Footprints cuando exista disponibilidad de datos para la misma región.
    \item Integrar información de radiación solar, cobertura eléctrica y demanda energética para obtener estimaciones más precisas del potencial fotovoltaico a nivel municipal.
    \item Mantener una infraestructura orientada a documentos (MongoDB) y formatos GeoJSON para facilitar futuras ampliaciones del sistema y permitir consultas espaciales eficientes sobre grandes volúmenes de información.
    \item Priorizar municipios PDET con alta concentración de edificaciones y condiciones favorables de radiación solar para maximizar el impacto de futuras inversiones en generación distribuida.
\end{itemize}

\section{Conclusión}

El proyecto diseñó e implementó una arquitectura NoSQL orientada al almacenamiento y procesamiento de información geoespacial para el análisis de potencial solar en territorios PDET. Mediante la integración del Marco Geoestadístico Nacional (MGN) y la generación de conjuntos de datos compatibles con MongoDB, se estableció una base sólida para realizar análisis espaciales a gran escala.

La evaluación comparativa de las fuentes de datos indicó que Google Open Buildings constituye una alternativa adecuada para la caracterización inicial de edificaciones en municipios priorizados, gracias a su amplia cobertura territorial y disponibilidad en zonas rurales. Microsoft Building Footprints desempeña un rol complementario valioso como mecanismo de validación y control de calidad en las áreas donde está disponible.

La solución propuesta proporciona una infraestructura escalable y reproducible que permite incorporar etapas posteriores de análisis, incluyendo la estimación de superficies útiles para la instalación de paneles solares, la generación de métricas energéticas y la priorización de territorios para la transición energética en Colombia.

En este sentido, el trabajo desarrollado constituye una herramienta de apoyo para la toma de decisiones de la UPME y demuestra el potencial de las tecnologías NoSQL para abordar problemas de análisis geoespacial asociados al desarrollo sostenible y la planificación energética nacional.
